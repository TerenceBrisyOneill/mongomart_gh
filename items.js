...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        callback(categories);
        */
        var categories = [], allNum = 0;

        this.db.collection('item').aggregate(
            [{$group:{"_id":"$category","num":{$sum:1}}},{$sort:{"_id":1}}]).toArray(
                function(err, docs){
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
        ...
        */
        ...
        var pageItem, pageItems = [], numSkip = 0;
         ...
         if(category == "All"){
            this.db.collection('item').aggregate(
            [{$match:{}},{$sort:{"_id":1}},{$skip:numSkip},{$limit:itemsPerPage}]).toArray(
                function(err, docs){
                    assert.equal(null, err);
                    assert.ok(docs.length > 0);

                    docs.forEach(function(pageItem){ 
                        pageItems.push(pageItem);
                    });
                    callback(pageItems);
                });
            ...
            this.db.collection('item').aggregate(
            [{$match:{"category":category}},{$sort:{"_id":1}},{$skip:numSkip},{$limit:itemsPerPage}]).toArray(
                function(err, docs){
                    assert.equal(null, err);
                    assert.ok(docs.length > 0);
    ...
    this.getNumItems = function(category, callback) {
        ...
        if(category == "All"){
            ...
            this.db.collection('item').aggregate(
            [{$match:{"category":category}}]).toArray(
                function(err, docs){
                    assert.equal(null, err);
                    assert.ok(docs.length > 0);
                    numItems = docs.length;
        ...
        */
        ...
        var item, items = [], numSkip = 0;
         ...
         if(page == 0){
            ...
            this.db.collection('item').aggregate(
            [{$match:{$text:{$search:query}}},{$sort:{"_id":1}},{$skip:numSkip},{$limit:itemsPerPage}]).toArray(
                function(err, docs){
                    assert.equal(null, err);
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        ...
        /*callback(numItems);*/
         ...
         this.db.collection('item').aggregate(
            [{$match:{$text:{$search:query}}}]).toArray(
                function(err, docs){
                    assert.equal(null, err);
    ...
    this.getItem = function(itemId, callback) {
        ...
        this.db.collection('item').aggregate(
            [{$match:{_id:itemId}}]).toArray(function(err,docs){
                assert.equal(null, err);
    ...
    this.addReview = function(itemId, comment, name, stars, callback) {
       ...
       /* var reviewDoc = {
        ...
        this.db.collection('item').updateOne({_id:itemId},{$push:{"reviews":reviewDoc}}, 
            function(err,result){
                assert.equal(null, err);
                //it appears that the result here is the updated doc
                callback(result);
        });
    }
